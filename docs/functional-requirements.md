...
Each of the four tabs owns a reusable `HSIViewer` instance. Once Super-Resolution has produced a temporary processed `HSIData` result, its Original/Processed control selects the active dataset for the whole application: Visualization recomputes RGB/indices and Hypercube data, Classification displays or computes the matching low/high-resolution result, and spectrum/pixel inspection reads the selected cube. Classification retains independent result and layer state for the original and processed datasets. The viewer supports pan/zoom, point annotation (with dormant box-prompt support), a per-pixel value overlay, spectrum requests, and rectangular crop requests. Pan/zoom is preserved across same-size mode changes and transferred between 2D tabs with coordinate scaling when resolutions differ. Dimension-changing crops refit the image, and crop undo/redo is connected at the main-window level.

The current UI is a 4-tab `QTabWidget` (Visualization / Super-Resolution / Calibration / Classification). Visualization switches between its 2D viewer and a dedicated PyQt6/OpenGL Hypercube widget. Classification adds a collapsible class-layer inspector beside its viewer. Hypercube preparation, SR inference/preview rendering, and both classification paths have background workers; source import and eager 2D visualization computation remain synchronous. Cropping is available from the 2D viewers' context menus, but is blocked during SR or classification and whenever the processed SR dataset is selected. `docs/architecture.md` describes an earlier sidebar/panel-swap design that was superseded by this tab-based layout. Treat that part of the architecture document as historical.
...
Additional implemented inspection aids (not new MVP acceptance criteria): Show Pixel Values displays numeric readings with swatches from each mode's displayed color; Classification also reports the class ID, while the SR viewer supplies only RGB for its selected dataset. Index Mean opens a non-modal dialog showing the active visualization result's global mean, observed min/max, and colormap gauge. It does not compute selection/mask statistics; the action is blocked only when invoked directly from the SR viewer while Processed is selected, but remains available from Visualization for the processed index result.
...
- Original/Processed is a global data-source toggle: Visualization modes, Hypercube, spectrum/pixel lookup, Classification, and Save Image use the selected dataset. Classification stores separate low/high-resolution results and layer settings; rerunning SR discards only the stale high-resolution classification. Loading/cropping/undoing the source invalidates old SR output and both classification slots; cancellation or failure retains the previous successful SR result.
