...
| Given a classification result, when the user shows, hides, solos, or renames a class layer, then the RGB/index display reflects the selected classes without changing analytical masks. | Model complete: `ClassificationLayerModel` exposes stable layer snapshots, individual masks/RGBA layers, visibility operations, and true-RGB/false-colour composites. View/Controller layer-panel widgets remain to be implemented. |
