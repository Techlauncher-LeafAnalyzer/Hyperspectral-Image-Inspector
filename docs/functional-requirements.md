...
Each of the four tabs owns a reusable `HSIViewer` instance. Once Super-Resolution has produced a temporary processed `HSIData` result, its Original/Processed control selects the active dataset for the whole application: Visualization recomputes RGB/indices and Hypercube data, Classification displays or computes the matching low/high-resolution result, and spectrum/pixel inspection reads the selected cube. Classification retains independent result and layer state for the original and processed datasets. The viewer supports pan/zoom, point annotation (with dormant box-prompt support), a per-pixel value overlay, spectrum requests, and rectangular or polygon crop requests. A polygon crop keeps a rectangular spectral cube plus a boolean region-of-interest (ROI) mask; excluded pixels render transparently. Pan/zoom is preserved across same-size mode changes and transferred between 2D tabs with coordinate scaling when resolutions differ. Dimension-changing crops refit the image, and crop undo/redo restores both cube geometry and ROI state.

The current UI is a 4-tab `QTabWidget` (Visualization / Super-Resolution / Calibration / Classification). Visualization switches between its 2D viewer and a dedicated PyQt6/OpenGL Hypercube widget. Classification adds a collapsible class-layer inspector beside its viewer. Hypercube preparation, SR inference/preview rendering, and both classification paths have background workers; source import and eager 2D visualization computation remain synchronous. Rectangle and polygon crop tools are available from the 2D viewers' context menus, but cropping is blocked during SR or classification and whenever the processed SR dataset is selected. `docs/architecture.md` describes an earlier sidebar/panel-swap design that was superseded by this tab-based layout. Treat that part of the architecture document as historical.
...
A headless `pytest`/`pytest-qt` suite is tracked in `ui_tests/` and runs in GitHub Actions on pushes to `main`, pull requests, and manual dispatches. It covers synthetic-cube import, visualization switching, rectangle and polygon crop/undo/redo, ROI-aware index values and RGB stretch, transparent polygon export, context-menu wiring, pan/zoom transfer, Hypercube geometry/controller/worker behavior, SR lifecycle/inference contracts, K-means worker integration, classification layer controls, and independent low/high-resolution classification state. Real-capture tests require locally supplied sample files. SR Model tests require optional PyTorch/SciPy dependencies, and the `sr_model` tests additionally use the tracked checkpoint to run real inference and check Qt responsiveness. CI installs only `requirements-dev.txt`, so optional SR dependency tests and real-capture tests skip there. The supervised classifiers and classification index-analysis API do not yet have dedicated tracked regression tests. Headless checks do not establish desktop OpenGL-driver compatibility. A **Done** status records code-level implementation and any stated regression coverage, not completed stakeholder validation or scientific-accuracy validation.
...
| Requirement | Status |
| --- | --- |
| Given a loaded cube, when the user selects RGB, NDVI, EVI, MCARI, MTVI, OSAVI, or PRI, then the corresponding per-pixel index is computed and rendered as a false-colour image. | Done: the radio buttons select cached Model results without resetting pan/zoom for unchanged dimensions. All six formulas exist, and switching Original/Processed recomputes the modes, pixel values, and Hypercube against the selected low/high-resolution dataset. Synthetic-cube UI tests cover every mode and the resolution swap; optional local-sample tests exercise real captures. |
...
| Given a loaded cube, when the user right-clicks a pixel and selects Spectrum Plot, then a reflectance-vs-wavelength plot for that pixel is displayed. | Done for plotting: the action opens a wavelength/value dialog for the pixel in the globally selected Original/Processed dataset. It plots stored values without performing calibration; raw input is not automatically reflectance. Reads are paused during SR and classification and serialized with Hypercube preparation. |

Additional implemented inspection aids (not new MVP acceptance criteria): Show Pixel Values displays numeric readings with swatches from each mode's displayed color; Classification also reports the class ID, while the SR viewer supplies only RGB for its selected dataset. Index Mean opens a non-modal dialog showing the active visualization result's global mean, observed min/max, and colormap gauge. After a polygon crop, index values outside the ROI are `NaN`, so the reported mean excludes them; this is still a global mean over the retained ROI rather than per-class statistics. The action is blocked only when invoked directly from the SR viewer while Processed is selected, but remains available from Visualization for the processed index result.

### 3.3 Cropping

| Requirement | Status |
| --- | --- |
| Given a loaded cube and a user-drawn or specified rectangular region, when the user confirms the crop, then a new cube containing only that region's data (all bands) is produced. | Done: context-menu Crop applies a dragged rectangle on mouse release, retains all bands in a lazy sub-image, recomputes source displays/Hypercube, and supports undo/redo. Dimension changes refit the 2D views. Source crop/undo/redo invalidates the old SR result and all classification results; crop is blocked while SR or classification runs and whenever the processed SR dataset is selected. UI tests cover drag signaling, recomputation, refitting, history, degenerate selections, and crop-to-export dimensions. |
| Given a loaded cube, when the user chooses Crop to Polygon and marks at least three vertices, then only pixels inside the polygon are retained for display and ROI-aware analysis, and the crop can be undone or redone. | Done: click to add vertices; double-click, right-click, or Enter to apply; Backspace removes the last vertex and Esc cancels. The spectral cube is cropped to the polygon's bounding box and stores a boolean ROI mask, not a physically non-rectangular cube. Successive polygon crops intersect their masks. Display is transparent outside the ROI; index values, index means, RGB stretch, and K-means clustering exclude those pixels. Supervised output masks exclude them, but its full target bounding box is still classified before masking. Undo/redo restores the ROI, and existing crop guards and downstream invalidation apply. Synthetic-cube tests cover these paths. |
...
| Requirement | Status |
| --- | --- |
| Given a loaded cube, a target number of classes, and a max-iterations value, when the user runs unsupervised classification, then the image is grouped into that many spectrally-similar segments and rendered as a labeled mask. | Done: `ClassificationService` runs SPy's K-means in a Controller-owned worker and returns an HxW class map plus class-first KxHxW one-hot masks. The Classification tab validates its inputs, reports progress, supports cancellation, warns about large memory estimates, and renders a colored label overlay. It classifies the selected Original/Processed dataset and keeps each resolution's result independent; after a polygon crop, only ROI pixels train K-means or receive class masks. Synthetic-cube tests exercise the worker and resolution-aware workflow. |
| Given a loaded cube, a ground-truth file, and a chosen classifier (Gaussian / Mahalanobis Distance / Perceptron), when the user runs supervised classification, then each pixel is labeled according to the trained classifier. | Partial: selecting one reference mask automatically resolves its paired hyperspectral cube. The selector now shows only the Model-supported choices with readable labels, Gaussian and Mahalanobis distance; these run in a worker and return HxW labels plus KxHxW one-hot masks. Pairing, mask dimensions, wavelengths, and sample counts are validated with user-facing errors. Polygon-excluded target pixels receive no class label in the final masks. Perceptron remains omitted pending an agreed dimensionality-reduction/training policy. |
| Given a classification result, when the user shows, hides, solos, or renames a class layer, then the RGB/index display reflects the selected classes without changing analytical masks. | Partial: the Model supports all four operations without mutating analytical masks. The collapsible layer inspector exposes per-class show/hide and opacity, show/hide all, overall opacity, and filled/borders-only rendering over the true-colour base image; changes are retained independently for Original and Processed results. Solo and rename exist in the Model but are not exposed in the UI. Layer-panel, compositing, debounce, and resolution-swap behavior have automated coverage. |
| Given a classification result and vegetation-index choice, when analysis runs, then each class has its own masked raster and finite-pixel summary statistics. | Model complete: NDVI, EVI, MCARI, MTVI, OSAVI, and PRI delegate to `VisualizationService` once per request, then produce per-class mean, median, standard deviation, minimum, maximum, counts, transparent display layers, and lazy masked rasters. Worker/UI presentation remains to be implemented. |
| Given the Segmentation panel is active, when the user Ctrl+clicks to place foreground/background points or Ctrl+drags a box, then those prompts feed the selected classifier as interactive input. | Partial: Ctrl+click point drawing and underlying point/box history methods exist in the viewer, but box mode is not exposed or activated by the UI, viewer undo/redo is not connected, and the implemented batch classifiers do not consume the prompts. |
...
- Original/Processed is a global data-source toggle: Visualization modes, Hypercube, spectrum/pixel lookup, Classification, and Save Image use the selected dataset. Classification stores separate low/high-resolution results and layer settings; rerunning SR discards only the stale high-resolution classification. Loading/cropping/undoing the source invalidates old SR output and both classification slots; cancellation or failure retains the previous successful SR result. After a polygon crop, SR processes its rectangular bounding box and maps the ROI mask onto the 2× result for display and later ROI-aware calculations; the mask does not reduce inference work.
...
| Given a computed visualization, calibration, or segmentation result, when the user selects Export, then the result is written as PNG/TIFF (images) or a labeled mask plus a CSV of per-segment statistics (segmentation). | Partial: File → Save Image exports the selected 2D RGB/index display, the selected Original/Processed RGB preview in SR, or the current composited classification view as PNG, TIFF, JPEG, or BMP for rectangular images. A polygon ROI is exported with transparent excluded pixels in PNG or TIFF; the current export path rejects JPEG and BMP for polygon crops because it does not preserve the ROI alpha mask in those formats. Hypercube has a separate Export current view control for a PNG/JPEG framebuffer capture; File → Save Image does not capture the 3D view. UI tests verify active-mode/SR preview pixels, transparent polygon export, unsupported-format errors, cancellation, empty-state handling, and cropped dimensions. Permanent full SR-cube export, calibration export, analytical classification masks, and per-segment CSV statistics are not implemented. |

## 4. Non-Functional Requirements

| Category | Target | Current state on `main` |
| --- | --- | --- |
| **Reliability** | Zero-crash tolerance on core operations (load, calibrate, visualize, classify, super-resolve). This is the direct pain point named by the client about the previous tool. All I/O and model-inference paths must catch exceptions and surface an error dialog instead of crashing. Session state auto-saves (via the JSON project file) after each significant action so a crash doesn't lose configuration. | Partial: import/spectrum/export errors are surfaced; Hypercube, SR, and classification workers report failures and handle cancellation. Classification validates parameters, training-file pairs, masks, wavelength alignment, sample counts, finite data, and SPy output; source-changing operations are blocked while it runs, and close waits asynchronously for cancellation. SR validates checkpoints and finite tensors, cleans incomplete output, preserves prior successful results, and avoids destroying an active thread on close. Session auto-save and calibration remain unimplemented, and the supervised classifier path lacks dedicated regression coverage. |
| **Performance** | Based on the largest real capture on hand (~900 MB, 480 bands, 1971×500 px, VNIR 352–899 nm, 12-bit): long-running operations (load, super-resolution, classification) run off the UI thread with progress feedback. Target: single-tray load completes in under 10 seconds on typical facility hardware. | Partial: Hypercube preparation, SR inference/RGB preview, and classification run in background workers with progress. SR uses bounded working-memory tiles, while K-means estimates its in-memory working set and warns at 1 GB. Import and eager RGB/index rendering still run on the GUI thread; waiting for a Hypercube reader to stop can also block it. A local CPU SR crop measurement is documented, but full-capture classification/SR runs and the under-10-second import target remain unverified. |
...
| **Usability** | Target user is a skilled technical operator, not a programmer. No onboarding flow is required, but every disabled control must show a tooltip explaining why. Keyboard shortcuts and multi-monitor polish are nice-to-haves. | Partial: Calibration has an unavailable tooltip; Hypercube has interactive camera/reset/export controls; SR offers Run/Cancel, progress, global Original/Processed comparison, and one-time resolution notices. Classification provides working actions, progress/cancellation, a resolution label, readable classifier choices, deterministic colours, and a collapsible layer inspector with visibility/opacity/global/border controls. Rectangle and polygon tools are available from the viewer menu, with a polygon preview and undoable edits. Crop refits after resizing, 2D comparison framing is preserved, pixel overlays include colour swatches/class IDs, and index means remain visible in a dialog. Disabled-control explanations are not yet consistent everywhere, and layer solo/rename are not exposed. |
